 ...
 * added to the main view. Assumptions: DisplayComponents should all be of type node and broken down
 * into relatively simple and well-organized responsibilities. Dependencies: Node
 * <p>
 * Example: When creating a button that has some responsibility in the view, this can be made as a
 * class that extends Display Component. Inside this class the abstract method
 * getDisplayComponentNode() can be overwritten to return the button, which the high level model can
 * then easily get by calling getDisplayComponentNode() on the class instance.
 *
 * @author Evan Kenyon
 */
public abstract class DisplayComponent {
   ...
   * be able to get the DisplayComponent node without being able to alter that node. This leaves the
   * responsibility of altering that specific DisplayComponent subclass to that subclass itself.
